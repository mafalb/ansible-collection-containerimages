...
centos7-systemd: deps
	ANSIBLE_COLLECTIONS_PATHS=../../..:playbooks/collections OS=centos7 ansible-bender build -l org.label-schema.build-date=$(DATE) org.opencontainers.image.created=$(DATE2) -- playbooks/systemd.yml $(if $(BASE_IMAGE), $(BASE_IMAGE), localhost/mafalb/centos7-base) $(if $(TARGET_IMAGE), $(TARGET_IMAGE), localhost/mafalb/centos7-systemd)
...
fedora36-systemd: deps
	ANSIBLE_COLLECTIONS_PATHS=../../..:playbooks/collections OS=fedora36 ansible-bender build -l org.opencontainers.image.created=$(DATE2) -- playbooks/systemd.yml $(if $(BASE_IMAGE), $(BASE_IMAGE), localhost/mafalb/fedora36-base) $(if $(TARGET_IMAGE), $(TARGET_IMAGE), localhost/mafalb/fedora36-systemd)
