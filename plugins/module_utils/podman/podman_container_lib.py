...
def update_options(opts_dict, container):
    ...
    for k in list(container):
        if k in aliases:
            ...
            key = k
        if ARGUMENTS_SPEC_CONTAINER[key]['type'] == 'list' and not isinstance(container[key], list):  # noqa: E501
            opts_dict[key] = [container[key]]
        elif ARGUMENTS_SPEC_CONTAINER[key]['type'] == 'bool' and not isinstance(container[key], bool):  # noqa: E501
            opts_dict[key] = to_bool(container[key])
        elif ARGUMENTS_SPEC_CONTAINER[key]['type'] == 'int' and not isinstance(container[key], int):  # noqa: E501
...
class PodmanContainer:
    ...
    def get_image_info(self):
        ...
        self.module.log("PODMAN-CONTAINER-DEBUG: %s" % type(out))
        return json.loads(out)[0] if rc == 0 else {}
